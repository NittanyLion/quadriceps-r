...
`build/update.sh` refreshes `inst/extdata/`, `RULES.md` and `NOTICE.md` here from the Julia
package, runs the tests, commits and pushes. **Never edit those three by hand**, and make
...
| path | what it holds |
|---|---|
| `R/catalog.R` | the catalog (`inst/extdata/index.tsv`, read on first use), rule loader, cache, `available_rules` |
...
| `inst/extdata/` | the rules and their catalog — generated |
